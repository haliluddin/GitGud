        ...
        if ($hasVariations) {
            foreach ($_POST as $key => $value) {
                if (preg_match('/^variation_name_(\d+)$/', $key, $matches)) {
                    $varIndex = $matches[1];
                    $variationsData[$varIndex]['title'] = $_POST["variation_title_{$varIndex}"] ?? "Variation {$varIndex}";
                    $variationsData[$varIndex]['rows'] = [];
        ...
        if (empty($productNameErr) && empty($productCodeErr) && empty($categoryErr) && empty($descriptionErr) &&
            ...
            if ($productId) {
                if ($hasVariations && isset($_POST['variation_name_1'])) {
                    foreach ($variationsData as $varIndex => $varData) {
                        $variationName = $varData['title'];
                        $variationId = $productObj->addVariations($productId, $variationName);
                        foreach ($varData['rows'] as $option) {
                            $addPrice = $option['addPrice'];
                            $subtractPrice = $option['subtractPrice'];
                            $optionStock = $option['initialStock'];
                            $variationImagePath = NULL;
                            if (!empty($option['imageBase64'])) {
                                $variationImagePath = $option['imageBase64'];
                            } else if (!empty($_FILES["variationimage_{$varIndex}"]["name"][0])) {
                                $variationImagePath = "uploads/" . basename($_FILES["variationimage_{$varIndex}"]["name"][0]);
                                move_uploaded_file($_FILES["variationimage_{$varIndex}"]["tmp_name"][0], $variationImagePath);
                            }
                            $variationOptionId = $productObj->addVariationOptions($variationId, $option['optionName'], $addPrice, $subtractPrice, $variationImagePath);
...
<style>
    ...
    .overlay i { flex-shrink: 0; width: 24px; max-width: 24px; }
</style>
            ...
            <script>
                ...
                function displaySelectedImage(variationFormId, rowId) {
                    ...
                    if (inputFile.files.length > 0) {
                        const formData = new FormData();
                        // Use the same key as in product image upload
