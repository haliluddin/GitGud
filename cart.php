...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_order'])) {
    ...
    $order_id = $cartObj->placeOrder($user_id, $payment_method, $order_type, $availableCart);
...
<main style="padding: 20px 120px;">
    ...
    <?php if(!empty($availableCart) || !empty($disabledCart)): ?>
        <?php if(!empty($availableCart)): ?>
            <?php foreach ($availableCart as $stallName => $items): 
...
<div class="modal fade" id="stockErrorModal" tabindex="-1" aria-labelledby="stockErrorModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="stockErrorModalLabel">Stock Issue</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        One or more items in your cart are no longer available in the desired quantity. Please adjust your cart.
      </div>
      <div class="modal-footer">
        <button type="button" id="stockOkButton" class="btn btn-secondary" data-bs-dismiss="modal">OK</button>
      </div>
    </div>
  </div>
</div>
...
document.getElementById('stockOkButton').addEventListener('click', function() {
    location.reload();
});
