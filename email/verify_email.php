...
<main class="wholewhole">
    <div class="bg-white border p-5 w-50 rounded-2 text-center">
        ...
        <div class="w-75 mx-auto my-4">
            <span class="small">If you do not receive the email within the next 3 minutes, use the button below to resend the verification email.</span>
        </div>
        <form method="POST">
            <input type="hidden" name="user_id" value="<?= $_SESSION['user']['id'] ?>" />
            <input type="hidden" name="first_name" value="<?= $user['first_name'] ?>" />
            <input type="hidden" name="email" value="<?= $email ?>" />
            <input type="submit" class="btn btn-primary w-50 p-3 rounded-5 resever" value="Resend Email" /> 
        </form>
    </div>
